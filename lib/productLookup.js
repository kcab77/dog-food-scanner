...
export async function saveProduct(barcode, productName, brand, ingredients, processingMethod) {
  try {
    ...
    if (barcode) {
      // Upsert by barcode so rescanning the same product updates it
      await supabase.from('products').upsert({ barcode, ...row }, { onConflict: 'barcode' })
    } else {
      // No barcode — insert by name, skip if duplicate
      await supabase.from('products').upsert({ ...row }, { onConflict: 'product_name', ignoreDuplicates: true })
    }
