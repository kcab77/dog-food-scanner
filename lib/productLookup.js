...
// Claude vision — image + text
async function claudeVisionFetch(base64Image, prompt) {
  return fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'x-api-key': ANTHROPIC_KEY,
...
// Look up product via Go-UPC API — returns ingredients directly
export async function lookupWithGoUPC(barcode) {
  if (!GOUPC_KEY) return null
  try {
    const response = await fetch(`https://go-upc.com/api/v1/code/${barcode}?key=${GOUPC_KEY}`)
    if (!response.ok) {
      console.log('Go-UPC lookup failed, status:', response.status)
      return null
    }
...
{"found": false}`
    ...
    const response = await claudeVisionFetch(base64Image, prompt)
    if (!response.ok) {
...
export async function lookupIngredientDetail(ingredientName) {
  ...
  // Fall back to Gemini
  try {
    const response = await claudeFetch(`Explain this pet food ingredient for a dog owner: "${ingredientName}"
...
Full ingredient list: ${ingredientList}`
    ...
    const response = await claudeFetch(prompt)
    const data = await response.json()
    return extractClaudeText(data) || "Sorry, I couldn't connect right now. Check your internet and try again."
  } catch (e) {
...
// Look up ingredient list for a product name or barcode
export async function lookupIngredientsWithClaude(productNameOrBarcode, brand) {
  try {
    ...
    const prompt = isBarcode
      ? `You are the backend ingredient extraction engine for PawGrade, a dog food scoring app.

Step 1 — Identify the product: Use UPC barcode ${productNameOrBarcode} to identify the product name and brand.

Step 2 — Find the ingredient list: Using the product name and brand, find the complete official ingredient list. Prioritize sources in this order: brand's official website, Chewy.com, Petco.com, other reputable pet retailers.

Step 3 — Extract ingredients only: Return ONLY the ingredients list in the exact order listed on the label. Ignore guaranteed analysis, feeding guidelines, and marketing copy.

Return ONLY this JSON, no preamble:
{
  "product_name": "exact product name",
  "brand": "brand name",
  "ingredients": "full comma-separated ingredient list exactly as on the label",
  "first_five_ingredients": ["ingredient1", "ingredient2", "ingredient3", "ingredient4", "ingredient5"],
  "processing_method": "kibble or raw or gently cooked or freeze-dried or baked or unknown",
  "aafco_status": "feeding_trials or nutrient_profile or null",
  "guaranteed_analysis": {
    "protein_pct": number or null,
  ...
  "source_url": "url where ingredients were found or empty string",
  "found": true,
  "fallback_needed": false
}

If you cannot find this product return only: {"found": false, "fallback_needed": true}`
      : `You are the backend ingredient extraction engine for PawGrade, a dog food scoring app.

Find the complete official ingredient list for this dog food: "${productNameOrBarcode}"${brand ? ` by ${brand}` : ''}. Prioritize sources in this order: brand's official website, Chewy.com, Petco.com, other reputable pet retailers.

Return ONLY the ingredients list in the exact order listed on the label. Ignore guaranteed analysis, feeding guidelines, and marketing copy.
...
If you cannot find this product return only: {"found": false, "fallback_needed": true}`
    ...
    const text = extractClaudeText(data)
    if (!text) return { found: false, fallback_needed: true }
    return parseGeminiJSON(text)
  } catch (e) {
    console.log('Claude lookup failed:', e)
    return { found: false, fallback_needed: true }
  }
}
